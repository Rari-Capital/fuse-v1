...
interface IOneTokenV1Base is IERC20ExtendedUpgradeable {
    ...
    function changeMintMaster(address mintMaster_, address oneTokenOracle)
        external;
    ...
    function balances(address token)
        external
        view
        returns (uint256 inVault, uint256 inStrategy);

    function collateralTokenCount() external view returns (uint256);

    function collateralTokenAtIndex(uint256 index)
        external
        view
        returns (address);
...
interface IOneTokenV1 is IOneTokenV1Base {
    ...
    function updateMintingRatio(address collateralToken)
        external
        returns (uint256 ratio, uint256 maxOrderVolume);

    function getMintingRatio(address collateralToken)
        external
        view
        returns (uint256 ratio, uint256 maxOrderVolume);

    function getHoldings(address token)
        external
        view
        returns (uint256 vaultBalance, uint256 strategyBalance);
}
